...
class GrupoComidas:
    ...
    def agrega_comida(self, dimension,_grupo_agentes):
        comida = Comida(random.randint(0, dimension[0]), random.randint(0, dimension[1]), 2, 2,self.lista_comidas, _grupo_agentes)
        # print("bloop")
    ...
    def crear_comidas(self, LARGO, ALTO, _grupo_agentes):
        for _ in range(self.cantidad):
            comida = Comida(random.randint(0, LARGO), random.randint(0, ALTO), 2, 2, self.lista_comidas, _grupo_agentes)
            self.lista_comidas.append(comida)
